...
public class UnblockSegmentsForPlayersHandler : IRequestHandler<UnblockSegmentsForPlayersCommand>
{
    ...
    public UnblockSegmentsForPlayersHandler(ISegmentRepository segmentRepository, IPlayerRepository playerRepository, IPlayerService playerService, IPlayerSegmentActService playerSegmentActService, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Unit> Handle(UnblockSegmentsForPlayersCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            segment.UnblockPlayers(players);
            await _playerSegmentActService.CreateActWithHistoryAsync(PlayerSegmentActType.Unblock, playerIds, segment.Id, request.ByUserId, true);
        }
